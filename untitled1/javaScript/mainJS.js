const itemsList = [{
    name: 'חלב',
    price: 8.50,
    id: 1,
    image: ''
}, {
    name: 'ביצים',
    price: 12,
    id: 2,
    image: 'https://www.google.com/url?sa=i&url=https%3A%2F%2Ftowardsdatascience.com%2F3-numpy-image-transformations-on-baby-yoda-c27c1409b411&psig=AOvVaw1UNbr67F16xB6nzUVUvM-C&ust=1616177306453000&source=images&cd=vfe&ved=0CAIQjRxqFwoTCKCfouW3uu8CFQAAAAAdAAAAABAD'
}, {
    name: 'שוקו',
    price: 5.5,
    id: 3,
    image: ''
}, {
    name: 'חמאה',
    price: 10,
    id: 4,
    image: 'data:image/jpeg;base64,'
}, {
    name: 'חלב 4%',
    price: 10,
    id: 5,
    image: ''
}];
/*
* בניית מערך של אובייקטים
* */
let filteredItems = itemsList;
/*
*
*מגדירים מערך זמני שהוא זה שיהיה מוצג למשתמש ומאתחלים אותו במערך שיצרנו למעלה
* */
let list;
init = () => {
    /*
    * בונים פונקצית אתחול שתרוץ בתחילת העליה של האתר.
    * */
    filteredItems=filteredItems.sort((item,item2)=>{
        /*
        * מבצעים מיון על פי האות הראשונה בשם המוצר ולאחר מכן הופכים את המערך שיתחילמלמטה למעלה
        * */
        if(item.name.charAt(0)>item2.name.charAt(0)){
            return 1
        }else if(item.name.charAt(0)<item2.name.charAt(0)){
            return -1
        }
    }).reverse();
    list = document.getElementById("list");
    /*
    * מאתחלים א0html
    * */
    filteredItems.map((item,index) => {
        /*
        * רצים על מפה שמזמנת פונקציה שמאתחלת את הפריט
        * */
        addItemToHtml(item,index);
    });
}

addItemToHtml = (item,index) => {
    const container = document.createElement("div");
    const text = document.createElement("div");
    const button = document.createElement("button");
    const image = document.createElement("img");
    /*
    * מייצרים את הפריטים שאנחנו רוצים
    * */
    image.src = item.image
    button.innerHTML = "שלח לסל קניות"
    text.innerHTML = item.name + "<br>" + item.price;
    /*
    * מאתחלים את המאפיינים של התגיות שיצרנו בערכים שנרצה
    * */

    container.appendChild(text);
    container.appendChild(button);
    container.appendChild(image);
    /*
    * מוסיפים אל המשתנה container את כל המשתנים שהגדרנו למעלה
    * */
    if(index%2===0){
        container.className = "item";
    }else{
        container.className = "item2";
    }

    image.className="item_image";
    /*
    * מגדירים את שמות המחלקות של התגיות
    * */

    list.appendChild(container);
    /*
    * מוסיפים אל הרשימה את הcontainer
    * */
}

myFunction = (text) => {
    list.innerHTML = '';

    /*
    *
    * מאפסים את הרשימה
    * */
    filteredItems = itemsList.filter((item, index) => {
        /*
        * מבצעים השמה אל המשתנה filteredItems את המערך הקבוע שלנו בסינון על פי פרמטרים מסוימים
        * */
        if (item.name.includes(text.target.value) || item.price.toString().includes(text.target.value)) {
            addItemToHtml(item, index);
        }
        return item.name.includes(text.target.value)
    });
}

function checkCookie() {
    var user=getCookie("firstName");
    if (user !== "") {
        alert("Welcome again " + user);
    } else {
        user = prompt("Please enter your name:","");
        if (user !== "" && user != null) {
            setCookie("firstName", user, 30);
        }
    }
}
function languageSection(form) {

    let newIndex = form.language.selectedIndex;
    let lang;
    if (newIndex === 0) {
        alert("Please Select Your Language!");
    } else {
        lang = form.language.options[newIndex].value;
        window.location.assign(lang);

    }
}




